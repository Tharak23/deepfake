...
export async function GET() {
  try {
    ...
    const verifiedResearchers = await db.collection('users').find({ 
      ...
      position: 1,
      field: 1
    }).sort({ verificationDate: -1 }).toArray();
    ...
    const researchers = verifiedResearchers.map(researcher => ({
      ...
      dateVerified: researcher.verificationDate ? new Date(researcher.verificationDate).toISOString() : new Date().toISOString(),
      institution: researcher.institution || 'Not specified',
      position: researcher.position || 'Researcher',
      field: researcher.field || 'Computer Science'
    }));
    ...
    console.error('Error fetching verified researchers:', error);
    return NextResponse.json(
      { error: 'Failed to fetch verified researchers', details: (error as Error).message },
      { status: 500 }
    );
  }
}
