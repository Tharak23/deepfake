...
import DeepFakeArticle from '@/models/DeepFakeArticle';
...
/**
 * POST /api/deepfake-news/seed
 * Seeds the database with dummy deepfake articles
 */
  ...
  try {
    ...
    for (const article of dummyArticles) {
      try {
        ...
        // Create new article
        await DeepFakeArticle.create(article);
        created++;
      } catch (error: any) {
        if (error.code === 11000) {
          // Duplicate key error
          ...
          errors.push(`Error creating article "${article.title}": ${error.message}`);
        }
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Dummy articles seeded successfully',
      created,
      skipped,
    ...
    console.error('Error seeding dummy articles:', error);
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to seed dummy articles',
